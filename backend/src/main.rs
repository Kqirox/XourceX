...
async fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    xourcex_backend::DataRetentionService::start_archive_worker(db_pool.clone());
    ...
    let yield_service =
        Arc::new(xourcex_backend::yield_service::DefaultOnChainYieldService::new());
    let interest_reconciliation = Arc::new(xourcex_backend::InterestReconciliationService::new(
        db_pool.clone(),
        yield_service,
    ...
    let lending_notification_service = std::sync::Arc::new(
        xourcex_backend::LendingNotificationService::new(db_pool.clone()),
    );
