use xourcex_backend::legacy_content::{LegacyContentService, UploadMetadata};
use sqlx::PgPool;
use uuid::Uuid;
...
async fn test_list_user_content(pool: PgPool) -> sqlx::Result<()> {
    ...
    let filters = xourcex_backend::legacy_content::ContentListFilters {
...
async fn test_filter_by_content_type(pool: PgPool) -> sqlx::Result<()> {
    ...
    // Filter by video
    let video_filters = xourcex_backend::legacy_content::ContentListFilters {
    ...
    // Filter by audio
    let audio_filters = xourcex_backend::legacy_content::ContentListFilters {
