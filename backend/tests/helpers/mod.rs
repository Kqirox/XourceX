...
use xourcex_backend::{create_app, Config};
use serde_json::json;
...
impl TestContext {
    ...
    pub async fn from_env() -> Option<Self> {
        ...
        INIT.call_once(|| {
            let _ = xourcex_backend::telemetry::init_tracing();
        });
        ...
        let config = Config {
            ...
            rate_limit: xourcex_backend::config::RateLimitConfig::default_for_tests(),
            db_pool: xourcex_backend::config::DbPoolConfig::from_env_or_defaults(),
        };

        // Run migrations
        xourcex_backend::db::run_migrations(&pool)
            .await
            .expect("failed to run migrations");

        let prometheus_handle = xourcex_backend::get_or_install_recorder();
