# XourceX PostgreSQL Backup & Disaster Recovery Runbook

## Overview

This document describes the automated backup strategy, retention policy,
restoration procedure, and disaster-recovery (DR) plan for the XourceX
PostgreSQL database.
...
wal_level = replica
archive_mode = on
archive_command = 'aws s3 cp %p s3://my-bucket/xourcex-wal/%f'
restore_command = 'aws s3 cp s3://my-bucket/xourcex-wal/%f %p'
```
...
```json
{
  "Rules": [
    {
      "Id": "xourcex-daily-backups",
      "Filter": { "Prefix": "xourcex-backups/" },
      "Status": "Enabled",
      "Transitions": [
...
```bash
export RESTORE_DATABASE_URL="postgres://user:pass@host:5432/xourcex"
export RESTORE_DROP_DB=yes           # drops and re-creates the target DB
./backend/scripts/restore.sh /backups/xourcex-backup-20260424T023012Z.sql.gz
```
...
./backend/scripts/restore.sh /backups/xourcex-backup-20260424T023012Z.sql.gz.gpg
```

### 4.3 Restore to a staging environment for validation

```bash
export RESTORE_DATABASE_URL="postgres://user:pass@staging-host:5432/xourcex_staging"
./backend/scripts/restore.sh /backups/xourcex-backup-20260424T023012Z.sql.gz
...
cd infra/
terraform apply -target=aws_db_instance.xourcex_primary
```
...
aws s3 cp s3://my-bucket/xourcex-backups/<backup-file> /tmp/
...
```bash
curl https://api.xourcex.io/health
curl https://api.xourcex.io/health/db
```
