#!/usr/bin/env bash
# =============================================================================
# XourceX Backup Monitoring Script
# =============================================================================
...
alert() {
    ...
    if [[ -n "${ALERT_WEBHOOK_URL:-}" ]]; then
        curl -s -X POST "${ALERT_WEBHOOK_URL}" \
            ...
            -d "{\"text\": \"[XourceX Backup Alert] ${message}\"}" || true
    fi

    if [[ -n "${ALERT_EMAIL:-}" ]] && command -v sendmail &>/dev/null; then
        echo -e "Subject: [XourceX] Backup Alert\n\n${message}" \
            | sendmail "${ALERT_EMAIL}" || true
    fi
}

log "=== XourceX backup monitor starting ==="

# Find the most recent backup file
NEWEST="$(find "${BACKUP_DIR}" -name "xourcex-backup-*.sql.gz*" \
    -type f -printf "%T@ %p\n" 2>/dev/null \
