#!/usr/bin/env bash
# =============================================================================
# XourceX PostgreSQL Backup Script
# =============================================================================
# Creates a compressed, timestamped pg_dump of the XOURCEX database.
# Designed to be run daily by cron or a container scheduler.
#
# Required environment variables:
#   BACKUP_DATABASE_URL   – full PostgreSQL connection string
#                           e.g. postgres://user:pass@host:5432/xourcex
#
...
#                           bucket prefix (e.g. s3://my-bucket/xourcex-backups)
...
#   30 2 * * * /opt/xourcex/scripts/backup.sh >> /var/log/xourcex-backup.log 2>&1
# =============================================================================
...
BACKUP_FILENAME="xourcex-backup-${TIMESTAMP}.sql.gz"
BACKUP_PATH="${BACKUP_DIR}/${BACKUP_FILENAME}"

# ── Pre-flight checks ─────────────────────────────────────────────────────────
log() { echo "[$(date -u +%Y-%m-%dT%H:%M:%SZ)] $*"; }

log "=== XourceX backup starting ==="
...
if [[ -n "${BACKUP_S3_BUCKET:-}" ]]; then
    ...
    if ! aws s3 cp "${BACKUP_PATH}" "${S3_KEY}" \
            ...
            --metadata "timestamp=${TIMESTAMP},source=xourcex-backup-script"; then
...
find "${BACKUP_DIR}" -name "xourcex-backup-*.sql.gz*" \
