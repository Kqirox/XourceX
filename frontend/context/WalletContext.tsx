...
export const WalletProvider = ({ children }: { children: React.ReactNode }) => {
  ...
  useEffect(() => {
    ...
    const savedAddress = localStorage.getItem("xourcex_wallet_address");
    const savedWalletId = localStorage.getItem("xourcex_wallet_id");
  ...
  useEffect(() => {
    const handleAddressChange = (e: Event) => {
      ...
      if (customEvent.detail?.address) {
        ...
        localStorage.setItem("xourcex_wallet_address", newAddress);
      }
    };
  ...
  const connectCustom = async (moduleId: string) => {
    ...
    try {
      ...
      setAddress(address);
      setSelectedWalletId(moduleId);
      localStorage.setItem("xourcex_wallet_address", address);
      localStorage.setItem("xourcex_wallet_id", moduleId);
      setIsModalOpen(false);
      router.push("/asset-owner");
    } catch (error) {
  ...
  const disconnect = useCallback(async () => {
    ...
    localStorage.removeItem("xourcex_wallet_address");
    localStorage.removeItem("xourcex_wallet_id");
    if (kit) {
      try {
