 ...
 * XourceX frontend (Next.js, Stellar wallet kit, Framer Motion, Google
...
function buildCSP(nonce: string): string {
  ...
  const fontSrc = ["'self'", "https://fonts.gstatic.com", process.env.CSP_FONT_SRC_EXTRA ?? ""].filter(Boolean).join(" ");
...
export function proxy(request: NextRequest): NextResponse {
  ...
  const hstsIncludeSubDomains = process.env.HSTS_INCLUDE_SUBDOMAINS !== "false"; // default true
  ...
  // the app; camera/microphone are not used by XourceX.
  const permissionsPolicy = process.env.PERMISSIONS_POLICY ?? "camera=(), microphone=(), geolocation=(), payment=(), usb=(), interest-cohort=()";
