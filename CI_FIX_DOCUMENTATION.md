...
```yaml
env:
  DATABASE_URL: postgres://postgres:postgres@localhost:5432/xourcex_test
...
# Start PostgreSQL
docker run -d \
  --name xourcex-test-db \
  -e POSTGRES_USER=postgres \
  -e POSTGRES_PASSWORD=postgres \
  -e POSTGRES_DB=xourcex_test \
  -p 5432:5432 \
  postgres:15

# Set environment variables
export DATABASE_URL=postgres://postgres:postgres@localhost:5432/xourcex_test
...
# Cleanup
docker stop xourcex-test-db
docker rm xourcex-test-db
```
